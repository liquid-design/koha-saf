...
def create_marcxml_record(book_data, isbn):
    ...
    # --- Items toevoegen (voorbeeld) ---
    # Later vul je barcode + branches dynamisch in
    add_item_fields(record, homebranch="CPL", holdingbranch="CPL", itemtype="BK")

    return record

def add_item_fields(record, homebranch, holdingbranch, itemtype):
    """
    Voeg Koha itemvelden toe aan MARCXML (952 tag).
    Je mag deze functie later uitbreiden.
    """
    ...
    record.add_field(
        Field(
            ...
            indicators=[' ', ' '],
            subfields=[
                #Subfield('p', barcode),         # barcode
                Subfield('a', homebranch),      # home branch
                Subfield('b', holdingbranch),   # holding branch
                Subfield('y', itemtype),        # item type
                Subfield('o', ''),              # prijs (optional)
            ]
        )
    )
